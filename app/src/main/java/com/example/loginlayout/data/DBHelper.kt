...
class DBHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        db.insert(TABLE_USERS, null, ContentValues().apply {
            put(COL_USERNAME, "admin"); put(COL_PASSWORD, hashPassword("admin123")); put(COL_IS_ADMIN, 1)
        })
        db.insert(TABLE_USERS, null, ContentValues().apply {
            put(COL_USERNAME, "user"); put(COL_PASSWORD, hashPassword("1234")); put(COL_IS_ADMIN, 0)
        })

        // Catalogo de juegos de muestra
        data class GameSeed(val title: String, val cat: String, val desc: String,
            val price: Double, val disc: Int, val imgRes: Int, val seller: String)
        val games = listOf(
            ...
            GameSeed("PIXEL DUNGEON X","RPG","Explora mazmorras generadas aleatoriamente en este clasico roguelike.",14.99,50,R.drawable.neon_streets,"retrodev"),
            ...
            GameSeed("MEGA FORCE 2","Platformer","Plataformas de accion con mas de 80 niveles y jefes finales epicos.",12.99,30,R.drawable.retro_storm,"retrodev"),
            GameSeed("CYBER QUEST","RPG","Un RPG cyberpunk por turnos con historia ramificada y multiples finales.",24.99,0,R.drawable.neon_streets,"retrodev"),
            GameSeed("TURBO KART 64","Racing","Carreras de karts con pistas tematicas retro y modo multijugador local.",14.99,0,R.drawable.shadow_blade,"retrodev"),
            GameSeed("NINJA LEGENDS","Action","Domina el arte del ninjutsu en este hack-and-slash de accion frenética.",19.99,25,R.drawable.retro_storm,"retrodev"),
            ...
            GameSeed("BEAT RUSH","Rhythm","Juego de ritmo con mas de 100 canciones de musica electronica retro.",7.99,0,R.drawable.retro_storm,"retrodev")
        )
        val now = System.currentTimeMillis()
        for (g in games) {
            db.insert(TABLE_PRODUCTS, null, ContentValues().apply {
                put(COL_TITLE, g.title); put(COL_CATEGORY, g.cat)
                put(COL_DESC, g.desc); put(COL_PRICE, g.price)
                put(COL_DISCOUNT, g.disc); put(COL_IMAGE_RES, g.imgRes)
                put(COL_SELLER, g.seller); put(COL_CREATED_AT, now)
            })
        }
    }
    ...
    fun validateUser(username: String, password: String): Pair<Boolean, Boolean> {
        ...
        val cursor = db.query(TABLE_USERS, arrayOf(COL_IS_ADMIN),
            "$COL_USERNAME = ? AND $COL_PASSWORD = ?", arrayOf(username.trim(), hashed),
            null, null, null)
        cursor.use {
            return if (it.moveToFirst()) Pair(true, it.getInt(it.getColumnIndexOrThrow(COL_IS_ADMIN)) == 1)
    ...
    fun insertProduct(p: Product): Long {
        val cv = ContentValues().apply {
            put(COL_TITLE, p.title); put(COL_CATEGORY, p.category); put(COL_DESC, p.description)
            put(COL_PRICE, p.price); put(COL_DISCOUNT, p.discount)
            put(COL_IMAGE_RES, p.imageRes); put(COL_IMAGE_PATH, p.imagePath)
            put(COL_SELLER, p.seller); put(COL_CREATED_AT, p.createdAt)
        }
        return writableDatabase.insert(TABLE_PRODUCTS, null, cv)
    }

    private fun cursorToProduct(it: Cursor) = Product(
        ...
        discount = it.getInt(it.getColumnIndexOrThrow(COL_DISCOUNT)),
    ...
    fun getAllProducts(): List<Product> {
        ...
        val cursor = readableDatabase.query(TABLE_PRODUCTS, null, null, null, null, null, "$COL_CREATED_AT ASC")
        cursor.use { if (it.moveToFirst()) do { list.add(cursorToProduct(it)) } while (it.moveToNext()) }
        return list
    }

    fun getDiscountedProducts(): List<Product> {
        val list = mutableListOf<Product>()
        val cursor = readableDatabase.query(TABLE_PRODUCTS, null, "$COL_DISCOUNT > 0", null, null, null, "$COL_DISCOUNT DESC")
        cursor.use { if (it.moveToFirst()) do { list.add(cursorToProduct(it)) } while (it.moveToNext()) }
        return list
    }

    fun getProductById(id: Int): Product? {
        val cursor = readableDatabase.query(TABLE_PRODUCTS, null, "$COL_ID = ?", arrayOf(id.toString()), null, null, null)
        cursor.use { return if (it.moveToFirst()) cursorToProduct(it) else null }
    }

    fun addToCart(productId: Int, quantity: Int = 1) {
        writableDatabase.insert(TABLE_CART, null, ContentValues().apply {
            put(COL_PRODUCT_ID, productId); put(COL_QUANTITY, quantity)
        })
    }
    ...
    fun getCartItems(): List<Pair<Int, Int>> {
        ...
        cursor.use { if (it.moveToFirst()) do {
            list.add(Pair(it.getInt(it.getColumnIndexOrThrow(COL_PRODUCT_ID)),
                         it.getInt(it.getColumnIndexOrThrow(COL_QUANTITY))))
        } while (it.moveToNext()) }
        return list
    }

    fun clearCart() { writableDatabase.delete(TABLE_CART, null, null) }
    ...
    fun purchaseCart() {
        ...
        try {
            for ((productId, _) in cartItems) {
                val exists = db.query(TABLE_PURCHASES, arrayOf(COL_ID), "$COL_PRODUCT_ID = ?",
                    arrayOf(productId.toString()), null, null, null).use { it.count > 0 }
                if (!exists) db.insert(TABLE_PURCHASES, null, ContentValues().apply {
                    put(COL_PRODUCT_ID, productId); put(COL_PURCHASED_AT, now)
                })
            }
    ...
    fun getPurchasedProducts(): List<Product> {
        val list = mutableListOf<Product>()
        val cursor = readableDatabase.rawQuery("""SELECT p.* FROM $TABLE_PRODUCTS p
